...
const EditField = ({ setTitle }) => {
  ...
  const value = window.pk
    ? window.pk
    : window.location.href.split("fields/")[1].split("/")[0];
  const field = useRef([]);
  ...
  const updateField = () => {
    ...
    usePatch(`/scanreportfields/${value}/`, data)
      .then((res) => {
        // redirect
  ...
  return (
    <div>
      <CCBreadcrumbBar>
        ...
        <Link
          href={`/scanreports/${scanReport.current.id}/tables/${table.current.id}/`}
        >
          {table.current.name}
        </Link>
        <Link
          href={`/scanreports/${scanReport.current.id}/tables/${table.current.id}/fields/${field.current.id}/`}
        >
          {field.current.name}
        </Link>
        <Link
          href={`/scanreports/${scanReport.current.id}/tables/${table.current.id}/fields/${field.current.id}/update/`}
        >
          Update
        </Link>
      </CCBreadcrumbBar>
      ...
      <VStack mt="20px" align="start">
        <Checkbox
          isChecked={isIgnore}
          onChange={(e) => setIsIgnore(e.target.checked)}
          isDisabled={!window.canEdit}
        >
          Is ignore
        </Checkbox>
        <Checkbox
          isChecked={passFromSource}
          onChange={(e) => setPassFromSource(e.target.checked)}
          isDisabled={!window.canEdit}
        >
          Pass from source
        </Checkbox>
