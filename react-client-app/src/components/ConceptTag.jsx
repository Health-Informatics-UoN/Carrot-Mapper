...
const ConceptTag = ({
  ...
  return (
    <Tooltip
      label={
        creation_type
          ? `${conceptId} ${conceptName} (${mapCreationTypeToString(
              creation_type,
            )})`
          : null
      }
    >
      ...
      >
        <Text w="max-content" ml="8px" py="4px">
          {conceptId + " " + conceptName}{" "}
          {creation_type ? `(${creation_type.charAt(0)})` : ""}
        </Text>
        {!readOnly && (
          <Wrap direction="right">
            <TagCloseButton
              onClick={() => {
                handleDelete(itemId, conceptIdentifier);
              }}
            />
          </Wrap>
        )}
