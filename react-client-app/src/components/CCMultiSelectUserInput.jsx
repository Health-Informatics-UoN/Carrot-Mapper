...
import {
    Flex, FormControl, FormLabel, FormErrorMessage, Select, Spinner
} from "@chakra-ui/react"

const CCMultiSelectUserInput = (props) => {
    /**
    ...
    if(props.isLoading){
        return <Select isDisabled={true} icon={<Spinner />} placeholder={'Loading '+props.id} />
    }
                ...
                {props.currentSelections.map((item, index) => {
                    return (
                        <div key={index} style={{ marginTop: "0px", marginBottom: "10px" }}>
                            <ConceptTag
                                conceptName={item.username}
                                conceptId={""}
                                conceptIdentifier={item.id}
                                itemId={item.id}
...
export default CCMultiSelectUserInput