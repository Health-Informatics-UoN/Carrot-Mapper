...
const UploadScanReport = ({ setTitle }) => {
  ...
  const [selectedDatasetProjectMembers, setSelectedDatasetProjectMembers] =
    useState([]);
  ...
  useEffect(async () => {
    ...
    setDataPartners([
      { name: "------" },
      ...dataPartnerQuery.sort((a, b) => a.name.localeCompare(b.name)),
    ]);
  ...
  useEffect(async () => {
    ...
    if (dataPartnerId != undefined) {
      ...
      const datasets_query = await useGet(
      ...
      setDatasets([
        { name: "------" },
        ...datasets_query.sort((a, b) => a.name.localeCompare(b.name)),
      ]);
  ...
  useEffect(async () => {
    ...
    if (selectedDataset.id) {
      ...
      const datasetProjects = await useGet(
        `/projects/?dataset=${selectedDataset.id}`,
      );
      let datasetProjectMembers = [
        ...new Set(datasetProjects.map((project) => project.members).flat()),
      ];
      if (selectedDataset.visibility === "RESTRICTED") {
        ...
        const validDatasetMembers = [
          ...new Set([
            ...selectedDataset.viewers,
            ...selectedDataset.editors,
            ...selectedDataset.admins,
          ]),
        ];
        datasetProjectMembers = datasetProjectMembers.filter((item) =>
          validDatasetMembers.includes(item),
        );
      }
      setSelectedDatasetProjectMembers(datasetProjectMembers);
      // remove already selected users that aren't in project members
      setScanreportViewers((viewers) =>
        viewers.filter((viewer) => datasetProjectMembers.includes(viewer.id)),
      );
      setScanreportEditors((viewers) =>
        viewers.filter((viewer) => datasetProjectMembers.includes(viewer.id)),
      );
    }
    setLoadingDatasetProjects(false);
  ...
  useEffect(async () => {
    ...
    if (projectList) {
      const full_projects = await useGet(
        `/projects/?name__in=${projects.map((project) => project.name).join()}`,
      );
      setProjectList((projectList) =>
        projectList.map((project) =>
          full_projects.find((el) => el.id === project.id)
            ? full_projects.find((el) => el.id === project.id)
            : project,
        ),
      );
  ...
  useEffect(async () => {
    if (projectList && activeUsersList) {
      ...
      setDatasetAdmins((currentAdminList) =>
        currentAdminList.filter((item) =>
          projects
            .map(
              (proj) =>
                projectList.find((project) => project.name === proj.name)
                  .members,
            )
            .flat()
            .includes(item.id),
      ...
      setDatasetEditors((currentEditorList) =>
        currentEditorList.filter((item) =>
          projects
            .map(
              (proj) =>
                projectList.find((project) => project.name === proj.name)
      ...
      setUsers((currentViewerList) =>
        currentViewerList.filter((item) =>
          projects
            .map(
              (proj) =>
                projectList.find((project) => project.name === proj.name)
  ...
  async function createDataset() {
    ...
    try {
      ...
      // revalidate
      const datasets_query = await useGet(
        `/datasets/?data_partner=${data_partner.id}`,
      );
      const projectsQuery = await useGet("/projects/?datasets=true");
      setProjectList(projectsQuery);
  ...
  const upload = async () => {
    try {
      ...
      if (whiteRabbitScanReport.name.split(".").pop() != "xlsx") {
        throw {
          statusText:
            "You have attempted to upload a scan report which is not in XLSX format. Please upload a .xlsx file",
        };
      }
      if (dataDictionary && dataDictionary.name.split(".").pop() != "csv") {
        throw {
          statusText:
            "You have attempted to upload a data dictionary which is not in csv format. Please upload a .csv file",
        };
      }
      ...
      formData.append(
        "visibility",
        scanReportIsPublic ? "PUBLIC" : "RESTRICTED",
      );
      if (!scanReportIsPublic) {
        // Only add viewers if scan report is RESTRICTED
        scanreportViewers.forEach((viewer) =>
          formData.append("viewers", viewer.id),
        );
      }
      scanreportEditors.forEach((editor) =>
  ...
  const removeScanreportViewer = (username) => {
    setScanreportViewers((pj) =>
      pj.filter((user) => user.username != username),
    );
  };
  const removeScanreportEditor = (username) => {
    setScanreportEditors((pj) =>
      pj.filter((user) => user.username != username),
    );
  };
  ...
  const filterProjectUsers = () => {
    return activeUsersList.filter((item) =>
      projects
        .map(
          (proj) =>
            projectList.find((project) => project.name === proj.name).members,
        )
        .flat()
        .includes(item.id),
    );
  ...
  function addDatasetViewer(newViewer) {
    ...
    setUsers((previous) => [
      ...previous.filter((user) => user.id !== newViewer.id),
      newViewer,
    ]);
  }
  ...
  return (
    <Container maxW="container.xl">
      ...
      <FormControl
        isInvalid={formErrors.datapartner && formErrors.datapartner.length > 0}
        mt={4}
      >
        <FormLabel htmlFor="Data Partner" w="200px">
          Data Partner
        </FormLabel>
        <Select
          value={JSON.stringify(selectedDataPartner)}
          onChange={(option) =>
            setselectedDataPartner(JSON.parse(option.target.value))
          }
        >
      ...
      <Box mt={4}>
        <FormLabel htmlFor="Dataset" w="200px">
          Dataset
        </FormLabel>
        {loadingDataset ? (
          <Select
          ...
          <Box>
            <FormControl
              isInvalid={
                formErrors.parent_dataset &&
                formErrors.parent_dataset.length > 0
              }
            >
              <Box display={{ md: "flex" }}>
                {projectList ? (
                  <Select
                    ...
                    onChange={(option) =>
                      setselectedDataset(JSON.parse(option.target.value))
                    }
                  >
                      ...
                      .map((item, index) => (
                        <>
                          <optgroup label={item.name}>
                            {datasets
                              .filter((dat) => item.datasets.includes(dat.id))
                              .map((item2, index2) => (
                                <option
                ...
                {addDatasetMessage ? (
                  <>
                    <CloseButton
                      ...
                      ml={"auto"}
                      mt={4}
                      isDisabled={addDatasetMessage == "Adding Dataset"}
                      onClick={closeAddingInterface}
                    />

                    <Flex alignItems={"center"} justifySelf="center" h="full">
                      {addDatasetMessage == "Adding Dataset" && <Spinner />}
                  ...
                  <>
                    ...
                    <Box mt="10px">
                      ...
                      <Flex alignItems={"center"}>
                        <Switch
                          isChecked={datasetVisibleToPublic}
                          onChange={(e) =>
                            setDatasetVisibleToPublic(
                              (restriction) => !restriction,
                            )
                          }
                        />
                        <Text fontWeight={"bold"} ml={2}>
                          {datasetVisibleToPublic ? "Public" : "Restricted"}
                        </Text>
                      </Flex>
                      {!datasetVisibleToPublic && (
                        <CCMultiSelectInput
                          ...
                          label={"Viewers"}
                          info={
                            "All Dataset admins and editors also have Dataset viewer permissions. If a Dataset is PUBLIC, then all users with access to any project associated to the Dataset will have Dataset viewer permissions."
                          }
                          isLoading={activeUsersList == undefined}
                          selectOptions={
                            activeUsersList
                              ? filterProjectUsers().map(
                                  (item) => item.username,
                                )
                              : []
                          }
                      ...
                      <CCMultiSelectInput
                        ...
                        label={"Editors"}
                        info={
                          "All Dataset admins also have Dataset editor permissions."
                        }
                        isLoading={activeUsersList == undefined}
                        selectOptions={
                          activeUsersList
                            ? filterProjectUsers().map((item) => item.username)
                            : []
                        }
                        currentSelections={datasetEditors.map(
                          (item) => item.username,
                        )}
                      ...
                      <CCMultiSelectInput
                        ...
                        label={"Admins"}
                        info={
                          "As the creator of this Dataset, you will automatically be one of its admins."
                        }
                        isLoading={activeUsersList == undefined}
                        selectOptions={
                          activeUsersList
                        ...
                        currentSelections={datasetAdmins.map(
                          (item) => item.username,
                        )}
                          ...
                          onChange={(option) =>
                            setProjects((pj) => [
                              ...pj.filter(
                                (proj) =>
                                  proj.name !=
                                  JSON.parse(option.target.value).name,
                              ),
                    ...
                    <Button w="full" mt={4} onClick={createDataset}>
                      Add new Dataset
                    </Button>
                  </>
                )}
      ...
      {!scanReportIsPublic && (
        <CCMultiSelectInput
          ...
          label={"Viewers"}
          info={
            "If the Scan Report is PUBLIC, then all users with access to the Dataset have viewer access to the Scan Report. Additionally, Dataset admins and editors have viewer access to the Scan Report in all cases."
          }
          ...
          selectOptions={
            activeUsersList
              ? activeUsersList
                  .filter((item) =>
                    selectedDatasetProjectMembers.includes(item.id),
                  )
      ...
      <FormControl
        isInvalid={formErrors.dataset && formErrors.dataset.length > 0}
        mt={4}
      >
        <FormLabel htmlFor="Scan Report name" w="200px">
          Scan Report name
        </FormLabel>
        <Input
          ref={scanReportName}
          onChange={() => setFormErrors({ ...formErrors, dataset: undefined })}
        />
      ...
      <FormControl
        ...
        mt={4}
      >
        <FormLabel htmlFor="WhiteRabbit ScanReport" w="200px">
          WhiteRabbit ScanReport
        </FormLabel>
        <input
          type="file"
          id="scanreport"
          onChange={readScanReport}
          style={{
            width: "100%",
            borderWidth: "1px",
            borderColor: "gray",
        ...
        />
        {formErrors.scan_report_file &&
          formErrors.scan_report_file.length > 0 && (
            <FormErrorMessage>
              {formErrors.scan_report_file[0]}
            </FormErrorMessage>
          )}
      </FormControl>
      ...
      <FormControl
        isInvalid={
          formErrors.data_dictionary_file &&
          formErrors.data_dictionary_file.length > 0
        }
        mt={4}
      >
        <FormLabel htmlFor="Data Dictionary" w="200px">
          Data Dictionary
        </FormLabel>
        <input
          type="file"
          id="datadictionary"
          onChange={readDataDictionary}
          style={{
            width: "100%",
            borderWidth: "1px",
            borderColor: "gray",
        ...
        {formErrors.data_dictionary_file &&
          formErrors.data_dictionary_file.length > 0 && (
            <FormErrorMessage>
              {formErrors.data_dictionary_file[0]}
            </FormErrorMessage>
          )}
      </FormControl>
