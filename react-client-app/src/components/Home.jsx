...
const Home = () => {
  ...
  const statusesRef = useRef(
    JSON.parse(window.status)
      .map((status) => status.id)
      .map((item) => ({ id: item, expanded: false, data: [] })),
  );
  ...
  useEffect(async () => {
    ...
    if (filter) {
      ...
      if (filter == "Active") {
        filteredReports = scanreportsRef.current.filter(
      ...
      filteredStatuses = statusesRef.current.map((status) => ({
        ...status,
        data: filteredReports.filter((report) => report.status == status.id),
      }));
      ...
      const last_24_hours = filteredReports.filter(
        (report) =>
          Math.round(new Date(report.created_at).getTime() / 1000) >=
          ts_24_hours_ago,
      );
      const last_week = filteredReports.filter(
        (report) =>
          Math.round(new Date(report.created_at).getTime() / 1000) >=
          ts_week_ago,
      );
      const last_month = filteredReports.filter(
        (report) =>
          Math.round(new Date(report.created_at).getTime() / 1000) >=
          ts_month_ago,
      );
      const last_3_month = filteredReports.filter(
        (report) =>
          Math.round(new Date(report.created_at).getTime() / 1000) >=
          ts_3_month_ago,
      );
      const last_year = filteredReports.filter(
        (report) =>
          Math.round(new Date(report.created_at).getTime() / 1000) >=
          ts_year_ago,
      );
      ...
      generatedCountStats = {
        ...
        scanreporttable_count: filteredReports
          .map((report) => report.scanreporttable_count)
          .reduce((a, b) => {
            return a + b;
          }, 0),
        scanreportfield_count: filteredReports
          .map((report) => report.scanreportfield_count)
          .reduce((a, b) => {
            return a + b;
          }, 0),
        scanreportvalue_count: filteredReports
          .map((report) => report.scanreportvalue_count)
          .reduce((a, b) => {
            return a + b;
          }, 0),
        scanreportmappingrule_count: filteredReports
          .map((report) => report.scanreportmappingrule_count)
          .reduce((a, b) => {
            return a + b;
          }, 0),
      };
  ...
  useEffect(async () => {
    ...
    const scanreportIds = chunkIds(
      scanreports.map((scanreport) => scanreport.id),
    );
    const countPromises = [];
    ...
    scanreports = scanreports.map((report) => ({
      ...report,
  ...
  const expandStatus = (id, expand) => {
    ...
    setStatuses((stat) =>
      stat.map((status) =>
        status.id == id ? { ...status, expanded: expand } : status,
      ),
    );
  };
  ...
  const setDonutData = (scanreport_data) => {
    ...
    data.labels = [
      ...new Set(scanreport_data.map((data) => data.data_partner.name)),
    ].sort((a, b) => a.localeCompare(b));
    // for each data partner, count the number of scan reports and add it to values
    data.labels.map((label) => {
      const values = scanreport_data.filter(
        (report) => report.data_partner.name == label,
      );
  ...
  const getGraphData = () => {
    ...
    const data = [];
    const dataPartners = [
      ...new Set(scanreportsRef.current.map((data) => data.data_partner.name)),
    ].sort((a, b) => a.localeCompare(b));
    ...
    <VStack w="100%">
      <VStack w="100%">
        <HStack>
          ...
          <Button
            variant={filter == "Active" ? "green" : "blue"}
            ml="10px"
            onClick={() => {
              setFilter("Active");
            }}
          ...
          <Button
            variant={filter == "Archived" ? "green" : "blue"}
            ml="10px"
            onClick={() => {
              setFilter("Archived");
            }}
          ...
          {scanreportDonutData && (
            <Container w={["100%", "100%", "100%", "50%"]}>
              <Plot
                ...
                layout={{
                  autosize: true,
                  title: "Scanreports by Data Partner",
                }}
                textinfo="text"
          ...
          {mappingrulesDonutData && (
            <Container w={["100%", "100%", "100%", "50%"]}>
              <Plot
                ...
                layout={{
                  autosize: true,
                  title: "Mapping rules by Data Partner",
                }}
                useResizeHandler={true}
                style={{ width: "100%", height: "auto" }}
              />
                ...
                <Td>
                  {item.expanded ? (
                    <ChevronUpIcon
                      ...
                      onClick={() => expandStatus(item.id, false)}
                    />
                  ) : (
                    <ChevronDownIcon
                      _hover={{ color: "blue.500" }}
                      onClick={() => expandStatus(item.id, true)}
                    />
                  )}
                </Td>
              </Tr>
              {item.expanded && (
                <Tr key={item.id}>
                  <Td colSpan="3">
                    <Table variant="striped" colorScheme="greyBasic">
                      <TableCaption placement="top">
                        Scan reports with status {mapStatus(item.id)}
                      </TableCaption>
                      <Thead>
                        <Tr>
                          <Th>ID</Th>
