...
const MappingTbl = (props) => {
  ...
  useEffect(() => {
    ...
    useGet(`/mappingruleslist/?id=${scan_report_id}`)
      .then((res) => {
        // not sure if this needs a / on the end or not as it's an undocumented endpoint
        ...
        newData = newData.filter(
          (rule) =>
            !rule.destination_field.name.includes("_source_concept_id") &&
            rule.term_mapping != null,
        );

        newData = newData.filter((item) => {
          const matches = newData.filter(
            (row) =>
              row.source_table.id === item.source_table.id &&
              row.source_field.id === item.source_field.id &&
              Object.keys(row.term_mapping)[0] ===
