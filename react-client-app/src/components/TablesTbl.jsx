      ...
      <div>
        <Flex padding="30px">
          ...
          <Flex marginLeft="10px">
            {loadingMessage ? loadingMessage : "Loading Tables"}
          </Flex>
        </Flex>
      </div>
    );
  ...
  return (
    <div>
      ...
      <Flex my="10px">
        ...
        <Spacer />
        <HStack>
          <Button variant="green" onClick={download_scan_report}>
            Download Scan Report File
          </Button>
          <Button
            variant="blue"
            isDisabled={window.hide_button}
            onClick={download_data_dictionary}
          >
            ...
            scanReportTables.map((item, index) => (
              <Tr key={index}>
                <Td maxW={"200px"}>
                  <Link
                    style={{ color: "#0000FF" }}
                    href={`/scanreports/${scanReportId}/tables/${item.id}/`}
                  >
                    {item.name}
                  </Link>
                </Td>
                <Td maxW={"200px"}>
                  {item.person_id ? item.person_id.name : null}{" "}
                </Td>
                <Td maxW={"200px"}>
                  {item.date_event ? item.date_event.name : null}
                </Td>
                {window.canEdit && (
                  <Td maxW={"200px"}>
                    <Link
