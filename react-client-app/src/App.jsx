...
const App = ({ page }) => {
  const handleDeleteConcept = (
    ...
    valuesRef.current = valuesRef.current.map((value) =>
      value.id == id ? { ...value, conceptsLoaded: false } : value,
    );
    ...
    useDelete(`/scanreportconcepts/${conceptId}`)
      .then(function (response) {
        ...
        getScanReportConcepts(id).then(async (scanreportconcepts) => {
          if (scanreportconcepts.length > 0) {
            ...
            useGet(
              `/omop/conceptsfilter/?concept_id__in=${conceptIds.join()}`,
            ).then((values) => {
            ...
            valuesRef.current = valuesRef.current.map((value) =>
              value.id == id
                ? { ...value, concepts: [], conceptsLoaded: true }
                : value,
            );
            setValues(valuesRef.current);
            setAlert({
      ...
      .catch(function (error) {
        ...
        valuesRef.current = valuesRef.current.map((value) =>
          value.id == id ? { ...value, conceptsLoaded: true } : value,
        );
        setValues(valuesRef.current);
        ...
        .then(async (response) => {
          ...
          if (response.detail == "Not found.") {
            ...
            setAlert({
              ...
              status: "error",
              title:
                "Concept id " + concept + " does not exist in our database.",
              description:
                "Response: " + response.status + " " + response.statusText,
            });
            onOpen();
          ...
          const omopTable = await useGet(
            `/omoptables/${destination_field.table}/`,
          );
          ...
          usePost(`/scanreportconcepts/`, data)
            .then(function (response) {
              ...
              getScanReportConcepts(id).then(async (scanreportconcepts) => {
                if (scanreportconcepts.length > 0) {
                  const conceptIds = scanreportconcepts.map(
                    (value) => value.concept,
                  );
                  useGet(
                    `/omop/conceptsfilter/?concept_id__in=${conceptIds.join()}`,
                    ...
                    const scan_report_concept = scanreportconcepts.filter(
                      (con) => con.concept.concept_id == concept,
                    )[0];
                    saveMappingRules(
                      scan_report_concept,
                      scanReportValue,
            ...
            .catch(function (error) {
              ...
              if (error.response && error.response.data) {
                ...
                description =
                  "Response: " + body.status_code + " " + body.statusText;
              }
              if (typeof error !== "undefined" && error.response != null) {
                setAlert({
                  status: "error",
  ...
  const getPage = () => {
    switch (page) {
      ...
      case "Values":
        return (
          <ValuesTbl
            handleDelete={handleDeleteConcept}
