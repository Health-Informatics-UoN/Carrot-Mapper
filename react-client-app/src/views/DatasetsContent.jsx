...
const DatasetsContent = (props) => {
  ...
  useEffect(async () => {
    ...
    try {
      ...
      let scanreports = await useGet(
        `/scanreports/?parent_dataset=${datasetId}`,
      );
      scanreports = scanreports.sort((b, a) =>
      ...
      scanreports.map((scanreport) => {
        ...
        created_at.displayString = moment(
          scanreport.created_at.toString(),
      ...
      for (let i = 0; i < authorIds.length; i++) {
        authorPromises.push(
          useGet(`/usersfilter/?id__in=${authorIds[i].join()}`),
        );
      }
      for (let i = 0; i < datasetIds.length; i++) {
        datasetPromises.push(
          useGet(`/datasets/?id__in=${datasetIds[i].join()}`),
        );
      }
      ...
      authors.forEach((element) => {
        scanreports = scanreports.map((scanreport) =>
          scanreport.author === element.id
            ? { ...scanreport, author: element }
            : scanreport,
        );
      });
      datasets.forEach((element) => {
        scanreports = scanreports.map((scanreport) =>
          scanreport.parent_dataset === element.id
            ? { ...scanreport, parent_dataset: element }
            : scanreport,
        );
      });
      ...
      for (let i = 0; i < dataPartnerIds.length; i++) {
        dataPartnerPromises.push(
          useGet(`/datapartners/?id__in=${dataPartnerIds[i].join()}`),
        );
      }
      ...
      if (dataPartners) {
        dataPartners.forEach((element) => {
          scanreports = scanreports.map((scanreport) =>
            scanreport.parent_dataset.data_partner === element.id
              ? { ...scanreport, data_partner: element }
              : scanreport,
          );
        });
      }
      ...
      activeReports.current = scanreports.filter(
        (scanreport) => scanreport.hidden === false,
      );
      archivedReports.current = scanreports.filter(
        (scanreport) => scanreport.hidden === true,
      );
      active.current
      ...
      const scanreportIds = chunkIds(
        data.current.map((scanreport) => scanreport.id),
      );
      const countPromises = [];
      for (let i = 0; i < scanreportIds.length; i++) {
        countPromises.push(
          useGet(
            `/countstatsscanreport/?scan_report=${scanreportIds[i].join()}`,
          ),
        );
      }
      const countStats = [].concat.apply([], await Promise.all(countPromises));
      data.current = data.current.map((report) => ({
        ...report,
        ...countStats.find((item) => item.scanreport === report.id),
      }));

      activeReports.current = data.current.filter(
        (scanreport) => scanreport.hidden === false,
      );
      archivedReports.current = data.current.filter(
        (scanreport) => scanreport.hidden === true,
      );
      active.current
        ? setDisplayedData(activeReports.current)
  ...
  const activateOrArchiveReport = (id, theIndicator) => {
    setDisplayedData((currentData) =>
      currentData.map((scanreport) =>
        scanreport.id === id ? { ...scanreport, loading: true } : scanreport,
      ),
    );
    data.current = data.current.map((scanreport) =>
      scanreport.id === id
        ? { ...scanreport, hidden: theIndicator }
        : scanreport,
    );
  ...
  const goToArchived = () => {
    if (active.current === true) {
      ...
      window.history.pushState(
        {},
        "",
        `/datasets/${datasetId}/?filter=archived`,
      );
      setTitle(`Scanreports in Dataset #${datasetId}`);
    }
  };
  ...
  const applyFilters = (variable) => {
    ...
    if (authorFilter !== "All") {
      newData = newData.filter(
        (scanreport) => scanreport.author.username === authorFilter,
      );
    }
    if (datapartnerFilter !== "All") {
      newData = newData.filter(
        (scanreport) => scanreport.data_partner.name === datapartnerFilter,
      );
    }
    if (datasetFilter !== "All") {
      newData = newData.filter(
        (scanreport) => scanreport.parent_dataset.name === datasetFilter,
      );
    }
    if (nameFilter !== "All") {
      newData = newData.filter(
        (scanreport) => scanreport.dataset === nameFilter,
      );
    }
    if (statusFilter !== "All") {
      newData = newData.filter(
        (scanreport) => scanreport.status === statusFilter,
      );
    }
    return newData;
  };
  ...
  const setStatus = (id, status) => {
    ...
    data.current = data.current.map((item) =>
      item.id === id ? { ...item, statusLoading: true } : item,
    );
    activeReports.current = data.current.filter(
      (scanreport) => scanreport.hidden === false,
    );
    ...
    usePatch(`/scanreports/${id}/`, patchData).then((res) => {
      data.current = data.current.map((item) =>
        item.id === id ? { ...item, status, statusLoading: false } : item,
      );
      activeReports.current = data.current.filter(
        (scanreport) => scanreport.hidden === false,
      );
  ...
  return (
    <div>
      ...
      <Link href={"/datasets/" + datasetId + "/details/"}>
      ...
      <HStack>
        ...
        {[
          { title: "Data Partner -", filter: datapartnerFilter },
          { title: "Dataset -", filter: datasetFilter },
          { title: "Name -", filter: nameFilter },
          { title: "Author -", filter: authorFilter },
          ...
          <Tr className={expanded ? "largeTbl" : "mediumTbl"}>
            ...
            <Th>
              <Select
                ...
                value="Dataset"
                readOnly
                onChange={(option) => setDatasetFilter(option.target.value)}
              >
                <option style={{ fontWeight: "bold" }} disabled>
                  Dataset
                </option>
                {[
                  ...[
                    ...new Set(
                      displayedData.map((data) => data.parent_dataset.name),
                    ),
                  ],
                ]
                  .sort((a, b) => a.localeCompare(b))
            ...
            <Th>
              <Select
                ...
                value="Name"
                readOnly
                onChange={(option) => setNameFilter(option.target.value)}
              >
                <option style={{ fontWeight: "bold" }} disabled>
                  Name
                </option>
            ...
            <Th>
              <Select
                ...
                value="Author"
                readOnly
                onChange={(option) => setAuthorFilter(option.target.value)}
              >
                <option style={{ fontWeight: "bold" }} disabled>
                  Author
                </option>
                {[
                  ...[
                    ...new Set(
                      displayedData.map((data) => data.author.username),
                    ),
                  ],
                ]
                  .sort((a, b) => a.localeCompare(b))
            ...
            <Th>
              <Select
                ...
                value="Status"
                readOnly
                onChange={(option) => setStatusFilter(option.target.value)}
              >
                <option style={{ fontWeight: "bold" }} disabled>
                  Status
                </option>
            ...
            {expanded && (
              <>
                <Th style={{ fontSize: "16px", textTransform: "none" }}>
                  Tables
                </Th>
                <Th style={{ fontSize: "16px", textTransform: "none" }}>
                  Fields
              ...
              <Tr className={expanded ? "largeTbl" : "mediumTbl"} key={index}>
                ...
                <Td maxW={"100px"}>
                  <Link
                    ...
                    href={`/scanreports/${item.id}/`}
                  >
                    {item.data_partner.name}
                  </Link>
                </Td>
                <Td maxW={"100px"}>
                  <Link
                    style={{ color: "#0000FF" }}
                    href={"/datasets/" + item.parent_dataset.id + "/details/"}
                  >
                    {item.parent_dataset.name}
                  </Link>
                </Td>
                <Td maxW={"100px"}>
                  <Link
                ...
                <Td maxW={"200px"} minW={expanded ? "170px" : "180px"}>
                  {item.created_at.displayString}
                </Td>
                <Td>
                  <Link href={"/scanreports/" + item.id + "/mapping_rules/"}>
                    <Button variant="blue">Rules</Button>
                  </Link>
                </Td>
                <Td>
                  <Link href={"/scanreports/" + item.id + "/details/"}>
                    <Button variant="blue" my="10px">
                      Details
                    </Button>
                  </Link>
                ...
                <Td>
                  {item.statusLoading === true ? (
                    ...
                    <Select
                      bg={mapStatusColour(item.status)}
                      color={mapStatusText(item.status)}
                      minW="max-content"
                      style={{ fontWeight: "bold" }}
                      variant="outline"
                      value={item.status}
                      onChange={(option) =>
                        setStatus(item.id, option.target.value)
                      }
                    >
                ...
                <Td>
                  <Link href={"/scanreports/" + item.id + "/assertions/"}>
                    <Button variant="green">Assertions</Button>
                  </Link>
                </Td>
                <Td textAlign="center">
                  {item.hidden ? (
                    <>
                      {item.loading ? (
                        ...
                        <ViewOffIcon
                          _hover={{ color: "blue" }}
                          onClick={() =>
                            activateOrArchiveReport(item.id, false)
                          }
                        />
                      )}
                    </>
                    ...
                    <>
                      {item.loading ? (
                        ...
                        <ViewIcon
                          _hover={{ color: "blue" }}
                          onClick={() => activateOrArchiveReport(item.id, true)}
                        />
                      )}
                    </>
                  )}
                ...
                {expanded && (
                  <>
                    <Td maxW={"100px"}>
                      {item.scanreporttable_count !== undefined
                        ? item.scanreporttable_count
                        : "counting"}
                    </Td>
                    <Td maxW={"100px"}>
                      {item.scanreportfield_count !== undefined
                        ? item.scanreportfield_count
                        : "counting"}
                    </Td>
                    <Td maxW={"100px"}>
                      {item.scanreportmappingrule_count !== undefined
                        ? item.scanreportmappingrule_count
                        : "counting"}
                    </Td>
                  </>
                )}
