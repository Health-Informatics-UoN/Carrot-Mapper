...
const ScanReportAdminForm = ({ setTitle }) => {
  ...
  useEffect(
    async () => {
      ...
      try {
        ...
        const [datasetsQuery, usersQuery, authorsQuery] = await Promise.all(
          queries,
        );
        // Get project members
        ...
        setSelectedDataset(
          datasetsQuery.find(
            (element) => element.id === scanReportQuery.parent_dataset,
          ),
        );
        setAuthor(authorsQuery);
        setIsPublic(scanReportQuery.visibility === "PUBLIC");
        setUsersList([
          authorsQuery,
          ...usersQuery
            .filter((user) => user.id != authorsQuery.id)
            .filter((user) => validUsers.includes(user.id)),
        ]);
  ...
  function handleVisibilitySwitch(newValue) {
    ...
    setScanReport({
      ...scanReport,
      visibility: newValue ? "PUBLIC" : "RESTRICTED",
    });
  }
