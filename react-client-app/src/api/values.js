...
const getValuesScanReportConcepts = async (
  ...
  scanreportconcepts = scanreportconcepts.filter(
    (concept) => concept.content_type == contentType,
  );
  ...
  const mappingRules = await useGet(
    `/mappingrulesfilter/?concepts__in=${scanreportconcepts
      .map((item) => item.id)
      .join()}`,
  );
  scanreportconcepts = scanreportconcepts.map((element) => ({
    ...element,
    mappings: mappingRules.filter((el) => el.concept == element.id),
  }));
