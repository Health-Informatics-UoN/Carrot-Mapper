...
class TestCanViewProject(TestCase):
    def setUp(self):
        ...
        self.user_with_perm = User.objects.create(username="gandalf", password="thegrey")
        # Give them a token
        Token.objects.create(user=self.user_with_perm)

        # Create user who cannot access the Project
        self.user_without_perm = User.objects.create(username="balrog", password="youshallnotpass")
        # Give them a token
    ...
    def test_project_member_can_view(self):
        ...
        force_authenticate(request, user=self.user_with_perm, token=self.user_with_perm.auth_token)
    ...
    def test_non_project_member_cannot_view(self):
        ...
        # Authenticate the user
        force_authenticate(request, user=self.user_without_perm, token=self.user_without_perm.auth_token)
