...
class ScanReportForm(forms.Form):
    ...
    def clean_scan_report_file(self):
        xlsx = Xlsx2csv(
            self.cleaned_data['scan_report_file'], outputencoding="utf-8")
        ...
        with open(filepath, "rt") as f:
            ...
            set_header = ['Table', 'Field', 'Description', 'Type', 'Max length', 'N rows', 'N rows checked',
                          'Fraction empty', 'N unique values', 'Fraction unique', 'Flag', 'Classification']
                ...
                for row in reader:
                    ...
                    if (flag_column == 'patientid') or (flag_column == 'date') or (flag_column == ''):
                        ...
                        raise (forms.ValidationError(
                            "Check Flag column values. Valid options are: 'PatientID', 'Date' or blank"))

                    if (classification_column in VOCABULARY_CHOICES.values()) or (classification_column == ''):
                        pass
                    else:
                        raise (forms.ValidationError("Check Classification column values. Valid options are:{}".format(
                            list(VOCABULARY_CHOICES.values()))))

                return self.cleaned_data['scan_report_file']
            else:
                raise (forms.ValidationError(
                    "Please check the following columns exist in the Scan Report: Table, Field, Description, Type, Max length, N rows, N rows checked, Fraction empty, N unique values, Fraction unique, Flag, Classification."))
...
class DocumentForm(forms.Form):
    ...
    def clean_document_file(self):
        data_dictionary_csv = self.cleaned_data['document_file'].read().decode(
            "utf-8-sig").splitlines()[0]
        header = data_dictionary_csv.split(',')
        column_names = ["Table Name", "Column Name",
                        "Column Description", "ValueCode", "ValueDescription"]

        if set(column_names) == set(header):
            return self.cleaned_data['document_file']
        else:
            raise (forms.ValidationError(
                "Please check your column names in your data dictionary"))
