...
class ScanReportCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = ScanReport
        fields = "__all__"
    ...
    def create(self, validated_data):
        ...
        form = ScanReportForm(validated_data, request.FILES)
        if form.is_valid():
            # Same logic from form_valid method
            parent_dataset = form.cleaned_data["parent_dataset"]
            if not (
                has_editorship(parent_dataset, request)
                or is_admin(parent_dataset, request)
            ):
